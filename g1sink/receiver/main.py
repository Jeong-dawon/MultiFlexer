# sender 공유 중지 후 재접속 가능
# playing, paused 상태 변화에 따른 변환 시간 측정 버전
# ================================================================
# Multi-Sender WebRTC Receiver (GStreamer + Socket.IO + PyQt5, Overlay)
# - 좌/우 전환 안정화 + 전환 시간(ms) 측정(fps-measurements 기반)
# - sender 종료/중지/끊김 시 개별 제거, 마지막이면 플레이스홀더 복귀
# - 하단 상태바 제거
# ================================================================
...
class PeerReceiver:
    def __init__(self, sio, sender_id, sender_name, ui_window: ReceiverWindow,
        ...
        self._on_ready = on_ready   # 전환 완료 콜백
        self._on_down  = on_down    # ICE 실패/끊김 콜백
        self._switch_t0 = None      # 전환 시작시각(ns)
        ...
        # QOS/ASYNC는 있어도 무방하지만, 전환 시간은 fps-measurements로 찍음
    ...
    # (옵션) 전환 완료(싱크 준비 완료) - 현재는 미사용
    def _on_async_done(self, bus, msg):
        if msg.src is self._display_bin and self._switch_t0 is not None:
            dt_ms = (time.time_ns() - self._switch_t0) / 1e6
            if self._on_ready:
                GLib.idle_add(self._on_ready, self.sender_id, dt_ms)
            self._switch_t0 = None

    # (옵션) QOS 기반 완료 - 현재는 미사용
    ...
    def on_incoming_stream(self, webrtc, pad):
        ...
        if fpssink:
            ...
            # ✅ 전환 시간 측정 + 기존 FPS 로깅을 한 콜백에서 처리
            def _on_fps(el, fps, drop, avg):
                # 전환 대기 중이면 첫 측정 시 완료 처리
            ...
            fpssink.connect("fps-measurements", _on_fps)
...
class MultiReceiverManager:
    ...
    def _set_active_sender(self, sid):
        # 선택 대상이 있다면 t0 없을 때도 즉시 세팅(초기 활성화 포함)
        t0 = self._last_switch_t0 or time.time_ns() if sid else None
        self.active_sender_id = sid
        for pid, peer in self.peers.items():
            if pid == sid:
                peer.set_visible(True, t0=t0)
            else:
                peer.set_visible(False)
        if sid:
    ...
    def _on_switch_ready(self, sid: str, dt_ms: float):
        if self.active_sender_id != sid:
            return  # 늦게 도착한 완료 이벤트는 무시
        name = self.peers[sid].sender_name if sid in self.peers else sid
        # 요청 포맷: [VIEW] switching sender: {이름}, time: {xx.x} ms
        print(f"[VIEW] switching sender: {name}, time: {dt_ms:.1f} ms")
    ...
    def switch_by_offset(self, offset: int):
        ...
        if self.active_sender_id not in self._order:
            self._last_switch_t0 = time.time_ns()
    ...
    def _bind_socket_events(self):
        ...
        @self.sio.on('remove-sender')  # 기존 서버 호환
...
# + 화면 전환 시간 측정 로그 추가(playing 상태 유지)
# 키보드 이벤트 발생 시 반사적으로 시간 측정 -> 변환할 sender가 없는 경우를 반영하지 못함 
# ================================================================
# Multi-Sender WebRTC Receiver (GStreamer + Socket.IO + PyQt5, Overlay)
# - Always PLAYING (모든 파이프라인 유지)
# - 좌/우 전환: 키 입력(t0) ~ 실제 표시(첫 handoff/QoS/async-done)까지 전환 시간 측정
# - 전환 완료 시: 
#     [VIEW] switching to sender: <name>
#     [VIEW] switching time: <ms> ms
# - sender 종료/중지/끊김 시 개별 제거, 마지막이면 플레이스홀더 복귀
# - 하단 상태바 제거, ↑/↓ 팝업 토글, Q 종료
# ================================================================
...
class PeerReceiver:
    def __init__(self, sio, sender_id, sender_name, ui_window: ReceiverWindow,
        ...
        self._on_ready = on_ready   # 전환 완료 콜백: (sid, dt_ms)
        self._on_down  = on_down    # ICE 실패/끊김 콜백
        self._switch_t0 = None      # 전환 시작 시각(ns), 선택된 순간에만 설정
        ...
        self._display_bin = None
        self._visible = True  # Always-Playing 모드에서 의미상 True
    ...
    # ----- 전환 완료 후보 1: 싱크 준비 완료(async-done) -----
    def _on_async_done(self, bus, msg):
        if self._switch_t0 is None:
            return
        # 어느 요소든 올 수 있으나, 첫 신호만 사용
        dt_ms = (time.time_ns() - self._switch_t0) / 1e6
    ...
    # ----- 전환 완료 후보 3: 첫 버퍼 handoff(identity) -----
    def _on_handoff(self, identity, buffer, pad=None, *args):
        # 일부 환경에서는 pad가 전달되지 않음 → pad를 선택 인자로 처리
        if self._switch_t0 is None:
            return
        dt_ms = (time.time_ns() - self._switch_t0) / 1e6
        self._emit_ready_once(dt_ms)
    ...
    def _emit_ready_once(self, dt_ms: float):
        # 여러 신호가 거의 동시에 올 수 있으므로 한 번만 콜백
        t0 = self._switch_t0
        self._switch_t0 = None
    ...
    def on_incoming_stream(self, webrtc, pad):
        ...
        # Always-Playing: 상태는 그대로 PLAYING 유지
        self._display_bin = fpssink
        print(f"[OK][{self.sender_name}] Incoming video linked → {decoder.name}")

    # Always-Playing: 상태 변경 없이 전환 시작 시각만 주입
    def set_visible(self, on: bool, t0: int = None):
        if on:
            self._switch_t0 = t0 if t0 is not None else time.time_ns()
...
class MultiReceiverManager:
    ...
    def _set_active_sender(self, sid):
        self.active_sender_id = sid
        # 선택된 peer에만 t0 전달(측정 시작). 다른 peer는 상태 유지(Always-Playing)
        for pid, peer in self.peers.items():
            if pid == sid:
                peer.set_visible(True, t0=self._last_switch_t0)
        if sid:
            name = self.peers[sid].sender_name if sid in self.peers else sid
            # 완료 로그는 PeerReceiver 쪽(on_ready)에서 찍음
            GLib.idle_add(self.ui.set_active_sender_name, sid, name)
        else:
            GLib.idle_add(self.ui.set_active_sender, sid)
        self._nudge_focus()
    ...
    def switch_by_offset(self, offset: int):
        ...
        if self.active_sender_id not in self._order:
            # 첫 활성화
            target_sid = self._order[0]
        else:
            cur = self._order.index(self.active_sender_id)
            nxt = (cur + offset) % len(self._order)
            target_sid = self._order[nxt]

        # 전환 시작(t0)과 시작 로그
        self._last_switch_t0 = time.time_ns()
        target_name = self.peers[target_sid].sender_name if target_sid in self.peers else target_sid
        # print(f"[VIEW] switching start: {target_name}")
