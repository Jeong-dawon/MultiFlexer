...
/*
// ======================================
// WebRTC Sender (화면 공유 송신자) — 소프트 중지/재개 + 기존 PC 재사용
//  - 중지 시: PC는 닫지 않고 sender.replaceTrack(null)로 송출만 끊음
//  - 재개 시: 기존 PC의 sender에 새 트랙 replaceTrack(track) (없으면 addTrack)
//  - 리시버가 re-offer을 보내면 answer로 응답 (pending offer 큐 처리 포함)
//  - 리시버가 sender-share-started/stopped를 인식해 파이프라인을 PAUSE/PLAY로 전환하는 설계에 맞춤
// ======================================
...
function getPc(id) {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate) {
      socket.emit('signal', { to: id, from: socket.id, type: 'candidate', payload: e.candidate });
    }
  };
  pc.oniceconnectionstatechange = () => console.log(`[SENDER] ICE (${id}):`, pc.iceConnectionState);
  pc.onconnectionstatechange   = () => console.log(`[SENDER] PC state (${id}):`, pc.connectionState);
...
function forceH264(pc) {
  try {
    pc.getTransceivers().forEach(t => {
      if (t.sender?.track?.kind === 'video') {
        const h264 = RTCRtpSender.getCapabilities('video').codecs.find(c => c.mimeType.toLowerCase() === 'video/h264');
        if (h264) t.setCodecPreferences([h264]);
      }
    });
  } catch(e) {}
...
// 새 스트림 트랙을 기존 PC에 붙이기 (가능하면 replaceTrack)
async function attachTracksToPc(pc, stream) {
  const vTracks = stream.getVideoTracks();
  const v = vTracks[0];
  if (!v) return;

  const senders = pc.getSenders ? pc.getSenders() : [];
  const vSender = senders.find(s => s.track && s.track.kind === 'video') || senders.find(s => s.track == null && s.transport);
  try {
    if (vSender) {
    ...
    console.warn('[SENDER] attachTracksToPc failed, fallback to addTrack:', e);
    try { pc.addTrack(v, stream); } catch(_) {}
  }
  forceH264(pc);
}

async function attachTracksToAllPcs(stream) {
  await Promise.all(Object.values(peerConnections).map(pc => attachTracksToPc(pc, stream)));
}

async function detachTracksFromAllPcs() {
  await Promise.all(Object.values(peerConnections).map(async (pc) => {
    try {
      for (const s of pc.getSenders?.() || []) {
        if (s.track) {
          try { await s.replaceTrack(null); } catch(_) {}
        }
      }
    } catch(_) {}
  }));
...
async function startLocalCaptureAndPreview() {
  ...
  try {
    ...
    // 트랙이 브라우저 쪽에서 종료되면 자동 중지 신호 날림
    localStream.getVideoTracks().forEach(t => {
      t.addEventListener('ended', () => {
        console.log('[SENDER] local track ended → auto stop');
        softStop();
      });
    });
...
async function announceShareAndProcessOffers() {
  ...
  // 기존 PC에 새 트랙 붙여두기 (리시버가 바로 보게)
  await attachTracksToAllPcs(localStream);
  ...
  for (const [rid, offer] of Object.entries(pendingOffers)) {
    try {
      ...
      // 트랙 연결(중복 방지하면서 replaceTrack 선호)
      await attachTracksToPc(pc, localStream);

      await flushPendingCandidates(rid);
      const answer = await pc.createAnswer();
      await pc.setLocalDescription(answer);
      socket.emit('signal', { to: rid, from: socket.id, type: 'answer', payload: { type: 'answer', sdp: answer.sdp } });
      console.log('[SENDER] answer 전송 →', rid);
...
socket.on('signal', async (data) => {
  ...
  if (data.type === 'offer') {
    try {
      ...
      if (!localStream) {
        const ok = await startLocalCaptureAndPreview();
        if (!ok) { pendingOffers[from] = data.payload; return; }
      }
      pendingOffers[from] = data.payload;
      ...
      // 가능한 replaceTrack, 없으면 addTrack
      await attachTracksToPc(pc, localStream);

      await flushPendingCandidates(from);
      const answer = await pc.createAnswer();
      await pc.setLocalDescription(answer);
      socket.emit('signal', { to: from, from: socket.id, type: 'answer', payload: { type: 'answer', sdp: answer.sdp } });
      console.log('[SENDER] answer 전송 →', from);
...
// ---------- 방/상대 상태 변화 처리(선택) ----------
socket.on('remove-sender', (rid) => { if (!rid) return; cleanupPc(rid, 'server-remove'); });
socket.on('sender-left', (d) => { const rid = d?.id || d?.senderId || d?.from; if (rid) cleanupPc(rid, 'left'); });
socket.on('sender-disconnected', (d) => { const rid = d?.id || d?.senderId || d?.from; if (rid) cleanupPc(rid, 'disconnected'); });

function cleanupPc(id, why='') {
  const pc = peerConnections[id];
  if (!pc) return;
  try { pc.close(); } catch(_) {}
  delete peerConnections[id];
  console.log('[SENDER] cleanup PC', id, why);
}

// ---------- 방 삭제 처리 ----------
socket.on('room-deleted', () => {
  alert('방이 삭제되었습니다.');
  Object.keys(peerConnections).forEach(id => cleanupPc(id, 'room-deleted'));
...
async function softStop() {
  clearStatsTimer();
  socket.emit('sender-share-stopped', { senderId: socket.id, name: senderName });

  // 1) 기존 PC들은 유지하되, 트랙만 분리
  await detachTracksFromAllPcs();
...
shareStopBtn.addEventListener('click', softStop);
